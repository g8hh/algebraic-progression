function staticMultipliers() {
  ...
  if(hasQU(1) && player.yChallenge != 5) mult = mult.mul(hasChargedQU(1) ? QUAD_UPGRADES[1].chargedEff() : QUAD_UPGRADES[1].eff())
...
function nonStaticMultipliers(points) {
  ...
  if(hasQU(13) && player.yChallenge != 5) mult = mult.mul(simulatedPoints.max(0).pow(hasZlabMilestone(2,1) ? 0.24 : 0.2).add(1))
  ...
  if(hasCU(0,3) && player.compChallenge != 10) mult = mult.mul(Decimal.pow(1.001,simulatedPoints.div(100000).mul(hasUpgrade(3) && player.challenge != 5 && player.compChallenge != 8 ? 2 : 1).mul(hasUpgrade(5) && player.challenge != 5 && player.compChallenge != 8 ? 1000000 : 1).mul(hasChallenge(5)?1e9:1).max(1).log(new Decimal(1).add(Decimal.div(0.11,xDivision()))).add(1).floor())) // non-static
...
function getFinalPoints(level){
  ...
  currentPointsValue = currentPointsValue.min(squareRootHardcap())
