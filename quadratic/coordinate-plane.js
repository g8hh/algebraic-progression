function sacEffect(x) {
  switch (x) {
    case 'x':
      let x = player.sacX.add(1).log(3).add(1).log2().div(hasQU(15) ? 1.5 : 3).mul(hasChargedQU(15) ? 1.5 : 1)
      ...
      x = x.add(1)
      if(x.gt(1e6)) x = x.div(1e6).pow(0.5).mul(1e6)
      if(player.challenge == 7 || inSqrtLevel(5)) x = new Decimal(1)
      return x
    break;
    case 'y':
      let base = player.sacY.mul(0.02)
      if(hasQU(14)) base = base.mul(hasChargedQU(14) ? 15 : 1.5)
    ...
    case 'x2':
      ...
      if(hasCU(1,1) && player.compChallenge != 10 && player.integration.challenge != 2) x2 = x2.mul(100)
      if(x2.gt(1e150)) player.x2.div(1e150).pow(0.7).mul(1e150)
      if(hasCU(0,2) && player.compChallenge != 10 && player.integration.challenge != 2) x2 = x2.mul(COMP_UPGRADES[2].eff())
      ...
      if(player.compChallenge != 10 && player.integration.challenge != 2) x2 = x2.pow(COMP_UPGRADES[14].eff())
      x2 = x2.pow(NumberSets.sacrificeValueEffects(2))
      if(player.compChallenge == 2) x2 = new Decimal(0)
      x2 = x2.mul(TemporalPlane.totalEffect())
      return x2
    break;
    case 'z':
      let z = player.sacZ.div(5).pow(1.5).add(1)
      if(z.gt(200)) z = z.div(200).pow(hasChargedQU(17) ? 0.31 : 0.3).mul(200)
...
function sacrifice(x){
  switch (x) {
    ...
    break;
    case 'x<sup>2</sup>':
      player.sacX2 = player.sacX2.add(player.x2.mul(hasCU(0,8) && player.compChallenge != 10 && player.integration.challenge != 2 ? COMP_UPGRADES[8].eff() : 1))
...
function slopeEffect() {
  ...
  mult = mult.pow(Y_CHALLENGES[5].eff())
...
if(player.challenge == 5 || player.compChallenge == 2) return new Decimal(1)
  switch (x) {
    ...
    case 2:
      return new Decimal(1).add(player.b.pow(1.2).div(10)).min(new Decimal(8.87).mul(COMP_CHALLENGES[2].eff()).mul(hasChargedUpgrade(8) && player.challenge != 5 && player.compChallenge != 8 ? 1.1 : 1).mul(IntegrationUpgrades.quadratic2.isBought() ? 1.1 : 1))
    case 3:
      return player.b.div(20).add(1).pow(1.2).sub(1).mul(hasChallenge(7)?1.05:1).min(new Decimal(2.72).mul(COMP_CHALLENGES[2].eff()).mul(hasChargedUpgrade(8) && player.challenge != 5 && player.compChallenge != 8 ? 1.1 : 1).mul(IntegrationUpgrades.quadratic2.isBought() ? 1.1 : 1))
  }
}
...
function transformEffect(x) {
  if (x == 1) {
    ...
    let x = new Decimal(1.05).pow(player.transformations.bought[2])
    if(x.gte(3)) x.div(3).pow(0.8).mul(3)
    x = x.mul(transformEffect(5))
    return x
  } else if (x == 3) {
    let x = Decimal.add(1,Decimal.mul(0.02,player.transformations.bought[3]))
    if(x.gte(1.4)) x.div(1.4).pow(0.8).mul(1.4)
    x = x.mul(transformEffect(5))
    return x
  } else if (x == 4) {
    let x = new Decimal(1.03).pow(player.transformations.bought[4])
    if(x.gte(1.3)) x.div(1.3).pow(0.8).mul(1.3)
    return x
  } else {
    return Decimal.add(1,player.transformations.bought[4].div(100)).min(hasChargedQU(12) ? 1.3 : 1.15)
  }
}

function transformCosts(x) {
  if(player.transformations.bought[x].gte(Decimal.add(40,SinusoidalUpgrades[26].eff())) && x != 1) {
